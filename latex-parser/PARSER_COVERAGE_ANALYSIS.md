...
当前解析器在**数学公式核心功能**方面达到 **100% 覆盖**，涵盖 20 个功能类别、372+ 项已实现特性，无功能缺失。
...
#### ❌ NodeLayout 缓存
- **方向**: 相同 AST 子树 + 相同 RenderContext → 复用 layout，避免重复测量
- **场景**: 长公式、频繁重绘场景（编辑器实时预览）
...
#### ❌ 增量测量
- **方向**: AST 部分变更时只重新测量受影响子树
- **场景**: WYSIWYG 编辑器的实时渲染性能
...
#### ❌ 基准测试套件
- **方向**: 建立标准化 benchmark（解析速度、测量速度、渲染帧率），跟踪性能回归
- **场景**: 持续集成
