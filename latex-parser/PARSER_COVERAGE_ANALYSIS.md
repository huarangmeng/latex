...
- ✅ 完整的装饰符号（帽子、波浪线、箭头、括号、取消线变体、花括号标注等23种）
...
| 状态 | 功能 | 命令 | 说明 |
|------|------|------|------|
| ⬜ | 四角标注 | `\sideset{_a^b}{_c^d}{\sum}` | 大型运算符四角上下标 |
| ⬜ | 交换图/矩阵箭头 | `\xrightarrow`, `\xhookrightarrow` 等 | 更多可扩展箭头变体 |
| ⬜ | 标签引用 | `\label{eq:1}`, `\ref{eq:1}`, `\eqref{eq:1}` | 跨公式引用系统 |
| ⬜ | 表格环境 | `\begin{tabular}` | 文本模式下的表格 |
| ⬜ | 张量/指标 | `\tensor`, `\indices` | 物理学张量表示法 |
| ⬜ | 定界符增强 | `\lvert`, `\rvert`, `\lVert`, `\rVert` | 额外的竖线定界符变体 |
| ⬜ | 数学字体增强 | `\symbf`, `\symsf`, `\symrm` (unicode-math) | Unicode 数学字体命令 |
