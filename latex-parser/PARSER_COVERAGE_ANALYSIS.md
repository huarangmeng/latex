...
#### 数学类代码（原子间距控制）
- ✅ `\mathord{内容}` 强制归类为普通符号（Ord）
- ✅ `\mathbin{内容}` 强制归类为二元运算符（Bin）
- ✅ `\mathrel{内容}` 强制归类为关系符号（Rel）
- ✅ `\mathopen{内容}` 强制归类为开定界符（Open）
- ✅ `\mathclose{内容}` 强制归类为闭定界符（Close）
- ✅ `\mathpunct{内容}` 强制归类为标点（Punct）
- ✅ `\mathinner{内容}` 强制归类为内部原子（Inner）
- 说明：这些命令改变内容子公式所属的 TeX 原子类，从而影响与相邻原子之间的数学间距（参见 TeXbook 第 18 章间距表）。渲染器 `MathSpacing` 会依据显式类代码计算间距。
...
## 21. physics / siunitx 常用子集

### ✅ 已支持

#### physics
- ✅ `\dv{x}`、`\dv{f}{x}`、`\dv[n]{f}{x}` 常微分简写
